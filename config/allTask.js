...
let taskLog = {}
const cronMap = {
  'jd_delCoupon.js':'X X * * *',//删除优惠券🎟（未设定自动运行，删券慎用）
  'jd_crazy_joy_coin.js':'X X * * *',//京东疯狂JOY挂机

  'jd_car_exchange.js': '0 8 * * *',//京东汽车兑换
  'jd_joy_reward.js': '0 8,16,24 * * *',//宠汪汪积分兑换京豆🐕
  'jd_ms.js': '0 8 * * *',//京东秒秒币
  'jd_cash_exchange.js': '0 8 * * *',//领现金兑换红包
    ...
    if (!taskLog[key]) {
      ...
      for (let i = 0; i < runFileList.length; i++) {
        ...
        if (isTheTime(thisFile, date)) {
          let code = 'node ' + thisFile
          runScript(code, thisFile)
          loggerCron(`时间${h}:${m}:${s} 定时执行${thisFile}`);
        }
      }
    }
  }, 1000);
...
function runScript(code, file) {
  let d = new Date()
  logger(`${d.getHours()}:${d.getMinutes()}:${d.getSeconds()} 执行脚本: ${code}`);
...
function doWriteFile(file, str, flag) {
  ...
  //   let d = new Date()
  //   logger(`${d.getHours()}:${d.getMinutes()}:${d.getSeconds()} 写入日志文件 ${path} 耗时：${time}`)
  // });
}
function logger(info) {
  ...
  // }
  // let date = new Date()
  // let loggerDate = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`
...
function loggerError(info) {
  ...
  // }
  // let date = new Date()
  // loggerDate = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`

  // fs.writeFile(`logs/3logger.error.${loggerDate}.txt`, '\n' + info, { 'flag': 'a' }, function (err) {

  // });
}
function logger2(info) {
  // if (!info) {
  //   return;
  // }
  // if (typeof info !== 'string') {
