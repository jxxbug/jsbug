...
!(async () => {
  ...
  for(let key  in codeJson){
    let ele = codeJson[key];
    if(Array.isArray(ele)){
      console.log(key+'   '+keyToCode[key])
      console.log(ele.join('@'))
      fileJson[keyToCode[key]] = ele.join('@')
      formatEnvStr += `${key}- ${keyToCode[key]}\n ${ele.join('@')};\n`
    }
  }
...
fs.writeFile('config/shareCodeJSON.md', formatEnvStr, { 'flag': '' }, function(err) {
  console.log(`生成互助码文件：config/shareCodeJSON.md`)
});
              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
                    ...
                    codeJson['东东工厂'] = codeJson['东东工厂'] || []
                    codeJson['东东工厂'].push(item.assistTaskDetailVo.taskToken)
                  }
                });
              }
            }
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
                  ...
                  codeJson['京喜工厂'] = codeJson['京喜工厂'] || []
                  codeJson['京喜工厂'].push(data.user.encryptPin)
                }
              } else {
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                if (data.active) {
                  ...
                  codeJson['京喜农场'] = codeJson['京喜农场'] || []
                  codeJson['京喜农场'].push(JSON.stringify(shareCodeJson))
                } else {
      ...
      try {
        if (err) {
          ...
          if (
            ...
            codeJson['京东萌宠'] = codeJson['京东萌宠'] || []
            codeJson['京东萌宠'].push($.petInfo.shareCode)
            console.log(
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if ($.taskList.filter(item => !!item && item['taskId']=== 3) && $.taskList.filter(item => !!item && item['taskId']=== 3).length) {
                ...
                codeJson['京东赚赚'] = codeJson['京东赚赚'] || []
                codeJson['京东赚赚'].push($.taskList.filter(item => !!item && item['taskId']=== 3)[0]['itemId'])
              }
            }
          }
        } catch (e) {
...
async function getPlantBean() {
  ...
  async function jdPlantBean() {
    ...
    if ($.plantBeanIndexResult.code === "0") {
      ...
      codeJson['种豆得豆'] = codeJson['种豆得豆'] || []
      codeJson['种豆得豆'].push($.myPlantUuid)
    } else {
      console.log(
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
      codeJson['京东农场'] = codeJson['京东农场'] || []
      codeJson['京东农场'].push($.farmInfo.farmUserPro.shareCode)
      console.log(
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data && data.data.userInviteCode) {
              ...
              codeJson['crazyJoy'] = codeJson['crazyJoy'] || []
              codeJson['crazyJoy'].push(data.data.userInviteCode)
            }
          }
        }
      } catch (e) {
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (data.data.bizCode === 0) {
            ...
            codeJson['闪购盲盒'] = codeJson['闪购盲盒'] || []
            codeJson['闪购盲盒'].push(invites && invites[0]['assistTaskDetailVo']['taskToken'])
          }
        } catch (e) {
          $.logErr(e, resp);
        } finally {
  ...
  return new Promise(async (resolve) => {
    $.get(taskUrl(`user/QueryUserInfo`), (err, resp, data) => {
      try {
        ...
        codeJson['财富岛'] = codeJson['财富岛'] || []
        codeJson['财富岛'].push(strMyShareId)
      } catch (e) {
        $.logErr(e, resp);
      } finally {
        resolve();
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.code===0 && data.data.result){
              ...
              codeJson['签到领现金'] = codeJson['签到领现金'] || []
              codeJson['签到领现金'].push(data.data.result.inviteCode)
            }
          }
        }
      } catch (e) {
